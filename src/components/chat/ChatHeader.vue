<template>
  <div class="border-b border-gray-800 px-4 py-2 hidden md:flex items-center gap-2">
    ...
    <div v-if="systemPrompt" class="flex items-center gap-1 text-sm text-gray-400" :title="t('header.customPrompt')">
    ...
    <div v-if="vectorStoreId" class="flex items-center gap-1 text-sm text-green-400" :title="t('header.rag')">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
        <path d="M9 4.804A7.968 7.968 0 005.5 4c-1.255 0-2.443.29-3.5.804v10A7.969 7.969 0 015.5 14c1.669 0 3.218.51 4.5 1.385A7.962 7.962 0 0114.5 14c1.255 0 2.443.29 3.5.804v-10A7.968 7.968 0 0014.5 4c-1.255 0-2.443.29-3.5.804V12a1 1 0 11-2 0V4.804z" />
      </svg>
      <span>RAG</span>
    </div>
    <div v-if="!useContext" class="flex items-center gap-1 text-sm text-yellow-400" :title="t('header.fast')">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
